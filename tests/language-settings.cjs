...
const buttons = ['en', 'id', 'ms', 'vi'].map((language) => ({
  dataset: {language},
  addEventListener(type, callback) { listeners[language + type] = callback; },
}));
const options = {
  innerHTML: '',
...
const document = {documentElement: {lang: 'en'}, createElement() { return overlay; }};
...
const factory = new Function('document', 'tr', 'let selectedLanguage = "en";' + source + '; return { createLanguageSheet, selected: () => selectedLanguage };')(document, () => 'Choose language');
let changes = 0;
const sheet = factory.createLanguageSheet(root, () => { changes++; });
sheet.open();
...
assert.match(html, /languageBtn\.addEventListener\("click", \(\) => languageSheet\.open\(\)\)/);
assert.match(html, /post = localizePost\(post\)/);
assert.match(html, /applyReaderLanguage\(\);\s*rerenderFeeds\(\);/);
for (const language of ['id', 'ms', 'vi']) {
  const start = html.indexOf(`    ${language}: {`, html.indexOf('const POST_I18N'));
  const end = language === 'vi' ? html.indexOf('\n    },\n  };', start) : html.indexOf('\n    },', start);
  const section = html.slice(start, end);
  for (let id = 1; id <= 15; id++) assert.match(section, new RegExp(`\\n\\s*${id}:\\s*\\[`), `${language} translates post ${id}`);
}
console.log('PASS: language button opens a four-option sheet and preserves the selection');
