...
assert.match(html, /Like to get more similar content\./);
assert.doesNotMatch(html, /You can also change language and adjust summary style\./);
assert.match(html, /data-tour-step="2"[\s\S]*Double tap the article or use the Bookmark button to bookmark it\./);
assert.match(html, /data-tour-step="2"[\s\S]*You can also adjust summary style here\./);
assert.match(html, /data-tour-step="2"[\s\S]*If you find the article interesting, you can share it using the Share button\./);
assert.match(html, /reader-tour-like-highlight[\s\S]*?I like this content/);
assert.doesNotMatch(html, /data-tour-step="1"\] \.reader-likebar/);
assert.match(html, /data-tour-step="2"\] \[data-reading-settings\][\s\S]*data-tour-step="2"\] \[data-bookmark\][\s\S]*data-tour-step="2"\] \[data-share\]/);
assert.match(html, /data-tour-step="1"\] \.reader-bottombar::before\s*\{[^}]*background:\s*#fff;/);
assert.match(html, /data-tour-step="2"\] \.bottombar-icons::after\s*\{[^}]*width:\s*126px;[^}]*background:\s*#fff;/);
